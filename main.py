import os
import json
from tkinter import Tk, Label, Button, Entry, filedialog, StringVar, Frame, BooleanVar, IntVar, Checkbutton, simpledialog, Toplevel
...
def process_images(api_key, max_size, resize_enabled, folder_entry):
...
def open_advanced_settings():
    ...
    Label(advanced_settings_window, text="カスタムプロンプトテンプレート:").pack(side="top", fill="x", padx=20, pady=10)
...
def main():
    ...
    Button(root, text="レシート一括処理開始", command=lambda: process_images(api_key_var.get(), max_size_var.get(), resize_enabled_var.get(), folder_entry)).pack(side="top", fill="x", padx=20, pady=10)
